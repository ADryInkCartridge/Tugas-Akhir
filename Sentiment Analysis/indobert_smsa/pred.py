...
class BertData():
    ...
    def preprocess_one(self, src_txt):
        ...
        # print(src_subtoken_idxs, segments_ids)
...
class Model(nn.Module):
    ...
    def forward(self, src, seg, mask_src):
        top_vec, pool = self.bert(input_ids=src, token_type_ids=seg, attention_mask=mask_src, return_dict=False)
        print("TOP_VEC: ", top_vec)
        print("TOP_VEC SHAPE: ", top_vec.shape)
        top_vec = self.dropout(top_vec)
        top_vec *= mask_src.unsqueeze(dim=-1).float()
        print("SQUEEZE:")
        print(mask_src.unsqueeze(dim=-1).float())
        print("TOP_VEC: ", top_vec)
        print("TOP_VEC SHAPE: ", top_vec.shape)
        print(torch.sum(top_vec, dim=1))
        top_vec = torch.sum(top_vec, dim=1) / mask_src.sum(dim=-1).float().unsqueeze(-1)
        print("SUM: ")
        print(torch.sum(top_vec, dim=1))
        print(mask_src.sum(dim=-1).float().unsqueeze(-1))
        print("TOP_VEC: ", top_vec)
        print("TOP_VEC SHAPE: ", top_vec.shape)
